...
const int number_of_int2_sensors = 0;
const int number_of_int3_sensors = 0;
int n_sensors = number_of_sensors;
//////////////////////////////////////////////////////////////////
...
Sensor* int2_sensor_ptrs[number_of_int2_sensors];
Sensor* int3_sensor_ptrs[number_of_int3_sensors];
Sensor** s_ptrs = sensor_ptrs;
...
void lowPower_withInts(unsigned long time_ms) {
    if(pinInterrupted) return;
...
// Sensor(nomenclature, is_analog, is_connected, is_low_power, pin_read, pin_power, pin_trigger=-1)   
  sensor_ptrs[0] = new HCSR04((char*)"D", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, 3, 5, 6);
  //int2_sensor_ptrs[0] = new HCSR04((char*)"D", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, 3, 5, 2);
...
pinMode(2, INPUT);
  ...
  PRINT_CSTSTR("LoRa sensor, extended version\n");
...
void loop(void)
{
  ...
  if (millis() > nextTransmissionTime || pinInterrupted) {
#else
  ...
  if(pinInterrupted==2){
      PRINT_CSTSTR("Interruption detected on pin 2\n");
      s_ptrs = int2_sensor_ptrs;
      n_sensors = number_of_int2_sensors;
  }
  else if(pinInterrupted==3){
      PRINT_CSTSTR("Interruption detected on pin 3\n");
      s_ptrs = int3_sensor_ptrs;
      n_sensors = number_of_int3_sensors;
  }
  else{
      s_ptrs = sensor_ptrs;
      ...
      if(pinInterrupted) sprintf(final_str, "%sI%i", final_str, pinInterrupted);
      
      //this is how we can wake up some sensors in advance in case they need a longer warmup time
      //digitalWrite(s_ptrs[4]->get_pin_power(),HIGH);
      ...
      for (int i=0; i<n_sensors; i++) {
          ...
          if(i==0 && pinInterrupted) continue;

          if (s_ptrs[i]->get_is_connected() || s_ptrs[i]->has_fake_data()) {

#ifdef STRING_LIB
              if (i==0) {
                  sprintf(final_str, "%s%s/%s", final_str, s_ptrs[i]->get_nomenclature(), String(s_ptrs[i]->get_value()).c_str());
              } 
              else {
                  sprintf(final_str, "%s/%s/%s", final_str, s_ptrs[i]->get_nomenclature(), String(s_ptrs[i]->get_value()).c_str());
              }
#else
              char float_str[10];            
              ftoa(float_str, s_ptrs[i]->get_value(), 2);
          
              if (i==0) {
                  sprintf(final_str, "%s%s/%s", final_str, s_ptrs[i]->get_nomenclature(), float_str);
              } 
              else {
                  sprintf(final_str, "%s/%s/%s", final_str, s_ptrs[i]->get_nomenclature(), float_str);
              }
