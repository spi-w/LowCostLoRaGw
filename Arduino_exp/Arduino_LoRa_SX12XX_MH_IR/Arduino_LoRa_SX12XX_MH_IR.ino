...
//IR Sensor active from the beginning
int sensorActivate=1;
...
//Beware, not all pin can be used for interruption (for nano or uno, only pins 2,3)   
  sensor = new MH_IR((char*)"ST", IS_NOT_ANALOG, IS_CONNECTED, IS_LOWPOWER, 3, A0);
//////////////////////////////////////////////////////////////////  
...
///////////////////////////////////////////////////////////////////
      ...
      if (sensorActivate){
          if(sensor->get_is_low_power()){
              digitalWrite(sensor->get_pin_power(),HIGH);
              delay(sensor->get_warmup_time());
          }
          attachInterrupt(digitalPinToInterrupt(sensor->get_pin_read()),action,RISING);
      }
      else{
          if(sensor->get_is_low_power())
              digitalWrite(sensor->get_pin_power(),LOW);
      }

      lowPower(waiting_t);

      //if (sensorActivate)
          detachInterrupt(digitalPinToInterrupt(sensor->get_pin_read()));
          
      if(sensorState)
        PRINT_CSTSTR("Sensor triggered\n");
...
///////////////////////////////////////////////////////////////////
      ...
      sprintf(final_str, "%s%s/%d", final_str, sensor->get_nomenclature(),sensorState);
      ...
      if (RXPacketL) {  
        ...
        if (i>=0 && message[i]=='/' && message[i+1]=='@') {
            ...
            switch ((char)cmdstr[i]) {
                  ...
                  case 'I':
                        sensorActivate = 1;
                        PRINT_CSTSTR("Sensor on\n");
                        break;
                    
                  case 'O':
                        sensorActivate = 0;
                        PRINT_CSTSTR("Sensor off\n");
                        break;
